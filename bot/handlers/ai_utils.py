...
def generate_slide_title(api_key, prompt, slide_number, previous_titles):
    ...
    try:
        ...
        context = " ".join(
            previous_titles[-3:])  # Use only the last 3 titles for context to avoid prompt getting too long
...
def create_presentation(prompt, num_slides, api_key_unsplash, api_key_openai):
    ...
    for slide_number in range(1, num_slides + 1):
        ...
        slide_title = generate_slide_title(api_key_openai, prompt, slide_number, previous_titles).replace(
            f"Slide {slide_number}:", "").strip()
        ...
        bullets = generate_unique_slide_content(api_key_openai, slide_title, prompt, max_bullets=4,
